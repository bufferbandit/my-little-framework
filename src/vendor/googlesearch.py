...
Ported to Python3 on July 1, 2017
bufferbandit
'''
...
class GoogleSearch:
    ...
    def search(self, query, num_results = 10, prefetch_pages = True, prefetch_threads = 10):
        ...
        for i in range(pages) :
            ...
            response.close()
            if total is None:
                totalText = next(soup.select(GoogleSearch.TOTAL_SELECTOR)[0].children).encode('utf-8')
                total = int(re.sub("[', ]", "", re.search("(([0-9]+[', ])*[0-9]+)", str(totalText)).group(1)))
